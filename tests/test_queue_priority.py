...
class TestQueuePriority(unittest.TestCase):
  ...
  def test_same_priority_preserves_fifo_order_by_filename(self):
    with tempfile.TemporaryDirectory() as tmp:
      queue_dir = Path(tmp)
      jobs = [
  ...
  def test_same_second_same_priority_does_not_claim_strict_fifo(self):
    with tempfile.TemporaryDirectory() as tmp:
      queue_dir = Path(tmp)
      jobs = [
        ("1700000001-aaa.json", {"type": "ScanChanged", "priority": "normal"}),
        ("1700000001-bbb.json", {"type": "ScanChanged", "priority": "normal"}),
        ("1700000001-ccc.json", {"type": "ScanChanged", "priority": "normal"}),
      ]
      for name, payload in jobs:
        (queue_dir / name).write_text(json.dumps(payload), encoding="utf-8")

      sorted_jobs = get_sorted_jobs(queue_dir)
      got = [p.name for p in sorted_jobs]
      self.assertEqual(len(got), 3)
      self.assertSetEqual(set(got), {
        "1700000001-aaa.json",
        "1700000001-bbb.json",
        "1700000001-ccc.json",
      })
